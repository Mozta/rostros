...
class Ui_CipherImageForm(object):
    ...
    def buscaRostros(self):
        ...
        for (x,y,w,h) in faces:
            ...
            cv2.putText(img,texto, (pos_x,pos_y), cv2.FONT_HERSHEY_SIMPLEX, 2, (255,255,255),2,cv2.CV_AA)
    ...
    *********************** Funciones para eigenFaces ***********************
    '''
    def crea_y_entrena_modelo(label_matrix):
    ...
    def predice_imagen(model, image):
        #Dado un modelo Eigenface, predice la etiqueta de una imagen
        return model.predict(image)

    def leer_csv(filename='faces.csv'):
        #Leer archivo csv
        csv = open(filename, 'r')
        return csv

    def prepara_entrenamiento_y_testing(file):
    ...
    def crea_etiquetas_matriz_dict(input_file):
        ...
        for line in input_file:
            ...
            if label_dict.has_key(int(label)):
                ...
                numpy.append(current_files,leer_matrix_from_file(filename))
            else:
                label_dict[int(label)] = leer_matrix_from_file(filename)

        return label_dict 

    def split_test_training_data(data, ratio=0.2):
        #Dividir una lista de archivos de imagen por el ratio del entrenamiento: datos de prueba
        test_size = int(math.floor(ratio*len(data)))
        random.shuffle(data)
        return data[test_size:], data[:test_size]

    def leer_matrix_from_file(filename):
